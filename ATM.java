...
public class ATM extends JFrame {
	...
	protected void buildApp() {
		...
		JButton withDraw3 = new JButton("Withdraw \u20B95000");
		bPConst.gridx = 0;
		bPConst.gridy = 2;
		...
		withDraw1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent event) {
				if (balance >= 500) {
					...
					readyToEnter = false;
					System.out.println("User Has Withdrawn Rs.500");
					updateTransactionHist("User Has Withdrawn Rs.500");
				} else {
					displayArea.setText("<html> Your Balance is below \u20B90. Unable to Withdraw!! <br><br>"
		...
		withDraw2.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent event) {
				if (balance >= 1000) {
					...
					displayArea.setText("<html>\u20B91000 Withrawn! <br><br>" + finishedTransaction() + "</html>");
					readyToEnter = false;
					System.out.println("User Has Withdrawn Rs.1000");
					updateTransactionHist("User Has Withdrawn Rs.1000");
				} else {
					displayArea.setText("<html> Your Balance is below \u20B90. Unable to Withdraw!! <br><br>"
							+ finishedTransaction() + "</html>");
				}
		...
		withDraw3.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent event) {
				if (balance >= 5000) {
					...
					readyToEnter = false;
					System.out.println("User Has Withdrawn Rs.5000");
					updateTransactionHist("User Has Withdrawn Rs.5000");
				} else {
					displayArea.setText(
							"<html> Insufficient Funds!!! Your Balance is below \u20B90. Unable to Withdraw!! <br><br>"
									+ finishedTransaction() + "</html>");
				}
			}
		});
			...
			public void actionPerformed(ActionEvent event) {
				if (readyToEnter == true) {
					if (Integer.parseInt(getInputSequence()) > 10000) {
						...
						displayArea.setText("<html> You have deposited \u20B9" + getInputSequence() + "! <br><br>"
								+ finishedTransaction() + "</html>");
